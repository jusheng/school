...
angular.module('app')
	.controller('sysdeskController', ['$rootScope', '$scope', '$http', '$state','$compile','$timeout','ngDialog','API','toaster','recordFormat',
		function($rootScope, $scope, $http, $state,$compile,$timeout,ngDialog,API,toaster,recordFormat) {

			$scope.app_name = "系统管理桌面";
			$scope.desk_name = "sysdesk";
			$scope.desk_bj = "sysdesk_bj"; //布局管理器class
			$scope.desk_lay_grid = "sysdesk_lay_grid"; //桌面容器class
			$scope.loading = true;

			$scope.exis_desk = true;
			$scope.exis_app = true;
			
			var desk_sign = 4; //桌面的标识 （解决多个桌面之间数据干扰）
			var iconapp_obj = {
