...
angular.module('app')
	.controller('oadeskController', ['$rootScope', '$scope', '$http', '$state','$compile','$timeout','ngDialog','API','toaster','recordFormat',
		function($rootScope, $scope, $http, $state,$compile,$timeout,ngDialog,API,toaster,recordFormat) {

			$scope.app_name = "办公OA桌面";
			$scope.desk_name = "oadesk";
			$scope.desk_bj = "oadesk_bj"; //布局管理器class
			$scope.desk_lay_grid = "oadesk_lay_grid"; //桌面容器class
			$scope.loading = true;

			$scope.exis_desk = true;
			$scope.exis_app = true;
			
			var desk_sign = 2; //桌面的标识 （解决多个桌面之间数据干扰）
			var iconapp_obj = {
