...
angular.module('app')
	.controller('resdeskController', ['$rootScope', '$scope', '$http', '$state','$compile','$timeout','ngDialog','API','toaster','recordFormat',
		function($rootScope, $scope, $http, $state,$compile,$timeout,ngDialog,API,toaster,recordFormat) {

			$scope.app_name = "资源管理桌面";
			$scope.desk_name = "resdesk";
			$scope.desk_bj = "resdesk_bj"; //布局管理器class
			$scope.desk_lay_grid = "resdesk_lay_grid"; //桌面容器class
			$scope.loading = true;

			$scope.exis_desk = true;
			$scope.exis_app = true;
			
			var desk_sign = 5; //桌面的标识 （解决多个桌面之间数据干扰）
			var iconapp_obj = {
