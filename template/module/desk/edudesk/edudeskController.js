...
angular.module('app')
	.controller('edudeskController', ['$rootScope', '$scope', '$http', '$state','$compile','$timeout','ngDialog','API','toaster','recordFormat',
		function($rootScope, $scope, $http, $state,$compile,$timeout,ngDialog,API,toaster,recordFormat) {

			$scope.app_name = "校务管理桌面";
			$scope.desk_name = "edudesk";
			$scope.desk_bj = "edudesk_bj"; //布局管理器class
			$scope.desk_lay_grid = "edudesk_lay_grid"; //桌面容器class
			$scope.loading = true;

			$scope.exis_desk = true;
			$scope.exis_app = true;
			
			var desk_sign = 3; //桌面的标识 （解决多个桌面之间数据干扰）
			var iconapp_obj = {
