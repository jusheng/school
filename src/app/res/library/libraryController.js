...
angular.module('app')
	.controller('libraryController', [ '$rootScope', '$scope', '$http','$timeout', '$state','toaster','recordFormat','API','Upload','ngDialog',
	                                function($rootScope, $scope, $http,$timeout, $state,toaster,recordFormat,API,Upload,ngDialog) {
	$scope.title="资源添加"+' ('+$state.params.subject_name+' '+$state.params.grade_name+')';
	...
	if($state.params.id){  //编辑
	   ...
	   $scope.title = "资源编辑"+' ('+$state.params.subject_name+' '+$state.params.grade_name+')';
	}else{
