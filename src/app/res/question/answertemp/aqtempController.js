...
angular.module('app')
	.controller('aqtempController', [ '$rootScope', '$scope', '$http','$timeout', '$state','toaster','recordFormat','API','Upload',
	                                function($rootScope, $scope, $http,$timeout, $state,toaster,recordFormat,API,Upload) {
	
    if(!$state.params.id){
        $scope.record_temp.aq_temp = {
            "c":"",
            "img":[]
        }
    ...
    $scope.onContentSelect = function(files){
    	...
    	angular.forEach(files,function(file){
    		...
    		file.upload.then(function(response){
                ...
                $scope.record_temp.aq_temp.img.push(response.data.data[0].imgUrl);
    		})
    	})
    };

    $scope.del_content_img = function(index){
        $scope.record_temp.aq_temp.img.splice(index,1);
    }
