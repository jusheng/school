			...
			var A = [],
				...
				C = {
					...
					open: function(f) {
						...
						var j = null;
						if (f = f || {}, !(n && f.name && (j = f.name + " dialog", this.isOpen(j)))) {
							var q = a.copy(b),
								D = ++e;
							...
							var G, H, I = a.extend({}, q.resolve);
							return a.forEach(I, function(b, c) {
								I[c] = a.isString(b) ? z.get(b) : z.invoke(b, null, null, c)
							}), t.all({
								...
								if (G = c('<div id="' + j + '" class="ngdialog' + g + '"></div>'), G.html(q.overlay ? '<div class="ngdialog-overlay"></div><div class="ngdialog-inner"><div class="ngdialog-content" role="document">' + e + "</div></div>" : '<div class="ngdialog-inner"><div class="ngdialog-content" role="document">' + e + "</div></div>"), G.data("$ngDialogOptions", q), F.ngDialogId = j, q.data && a.isString(q.data)) {
								...
								if (q.className && G.addClass(q.className), q.appendClassName && G.addClass(q.appendClassName), q.width) {
									var k = G[0].querySelector(".ngdialog-content");
									a.isString(q.width) ? k.style.width = q.width : k.style.width = q.width + "px"
								}
								if (q.disableAnimation && G.addClass(i), H = q.appendTo && a.isString(q.appendTo) ? a.element(document.querySelector(q.appendTo)) : A.body, B.applyAriaAttributes(G, q), q.preCloseCallback) {
									var l;
									a.isFunction(q.preCloseCallback) ? l = q.preCloseCallback : a.isString(q.preCloseCallback) && F && (a.isFunction(F[q.preCloseCallback]) ? l = F[q.preCloseCallback] : F.$parent && a.isFunction(F.$parent[q.preCloseCallback]) ? l = F.$parent[q.preCloseCallback] : v && a.isFunction(v[q.preCloseCallback]) && (l = v[q.preCloseCallback])), l && G.data("$ngDialogPreCloseCallback", l)
								}
								if (F.closeThisDialog = function(a) {
										B.closeDialog(G, a)
									}, q.controller && (a.isString(q.controller) || a.isArray(q.controller) || a.isFunction(q.controller))) {
									var n;
									q.controllerAs && a.isString(q.controllerAs) && (n = q.controllerAs);
									var p = y(q.controller, a.extend(f, {
										$scope: F,
										$element: G
									}), !0, n);
									q.bindToController && a.extend(p.instance, {
										ngDialogId: F.ngDialogId,
										ngDialogData: F.ngDialogData,
										closeThisDialog: F.closeThisDialog,
										confirm: F.confirm
									}), "function" == typeof p ? G.data("$ngDialogControllerController", p()) : G.data("$ngDialogControllerController", p)
								}
								if (w(function() {
										var a = document.querySelectorAll(".ngdialog");
									...
									var r = B.getRouterLocationEventName();
									v.$on(r, function() {
										B.closeDialog(G)
									})
								}
								if (q.preserveFocus && G.data("$ngDialogPreviousFocus", document.activeElement), d = function(a) {
									...
									var t = F.hammerTime = x.Hammer(G[0]);
									t.on("tap", d)
								} else G.bind("click", d);
								return o += 1, C
							}), {
