...
angular.module('ivh.treeview').directive('ivhTreeview', ['ivhTreeviewMgr', function(ivhTreeviewMgr) {
  ...
  return {
    ...
    scope: {
      ...
      onSelect: '&ivhTreeviewOnSelect',  //2016-9-21添加 (选取当前节点)
      onDel: '&ivhTreeviewOnDel',  //2016-9-21添加 （删除当前节点）
    ...
    controller: ['$scope', '$element', '$attrs', '$transclude', 'ivhTreeviewOptions', 'filterFilter', function($scope, $element, $attrs, $transclude, ivhTreeviewOptions, filterFilter) {
      ...
      trvw.sort = function(node) {
        return node[localOpts.sortAttribute].sort;
      };
      ...
      //2016-9-21添加（选取当前节点）
      trvw.select_this = function(node){
    
        if(localOpts.onSelect) {
          var locals = {
            ivhNode: node
          };
          localOpts.onSelect(locals);
        }


      }

    trvw.exit_del = function(){
      return localOpts.onDel;
    }  
    trvw.del_this = function(node){
        if(localOpts.onDel) {
          var locals = {
            ivhNode: node
          };
          localOpts.onDel(locals);
        }
      }
  ...
  var options = {
    ...
    nodeTpl: [
      '<div class="ivh-treeview-node-content" title="{{trvw.label(node)}}">',
        ...
        '</span>',
        '<span class="ivh-treeview-node-label" ivh-treeview-toggle ng-click="trvw.select_this(node)">',
          '{{trvw.label(node)}} ',
        '</span> <i ng-if="trvw.exit_del()" ng-click="trvw.del_this(node)">[删除] (s:{{trvw.sort(node)}})</i>',
